...
let slides;
try {const r=await fetch('./slides.json?v=20260907-closing');if(!r.ok)throw Error();slides=await r.json();if(slides.length!==123)throw Error();}
...
function render(n,updateHash=true){
 ...
 byId('slide-image').src=s.image+'?v=20260907-closing';byId('slide-image').alt=`Slide ${current}: ${s.title}`;byId('slide-number').value=current;
