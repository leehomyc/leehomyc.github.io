...
let slides;
try {const r=await fetch('./slides.json');if(!r.ok)throw Error();slides=await r.json();if(slides.length!==120)throw Error();}
...
function render(n,updateHash=true){
 ...
 byId('slide-image').src=s.image;byId('slide-image').alt=`Slide ${current}: ${s.title}`;byId('slide-number').value=current;
