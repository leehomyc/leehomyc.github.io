(() => {
  ...
  function populateSubmission(submission, code, studentId) {
    ...
    document.getElementById('identity-title').textContent = 'Edit Quiz 04.';
  ...
  function buildPayload() {
    ...
    return { action: 'quiz-submit', quizId: 'week-04', code: data.get('code'), name: data.get('name'), studentId: data.get('studentId'), answers: data.getAll('answer') };
  }
  ...
  function showPreview(payload) {
    ...
    confirmButton.textContent = isEditing ? 'Confirm and update Quiz 04 →' : 'Confirm and submit Quiz 04 →';
  ...
  lookupForm.addEventListener('submit', async event => {
    ...
    lookupResult.hidden = true;
    try {
      const result = await request({ action: 'quiz-lookup', quizId: 'week-04', code, studentId });
      if (!result.submitted) {
        lookupResult.innerHTML = '<p>No Quiz 04 submission was found for that student ID.</p><button type="button">Start a new submission →</button>';
