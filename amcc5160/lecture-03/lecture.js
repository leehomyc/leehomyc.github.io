...
let slides;
try {const r=await fetch('./slides.json?v=20260907-polished');if(!r.ok)throw Error();slides=await r.json();if(slides.length!==100)throw Error();}
...
function render(n,updateHash=true){
 ...
 const sectionStart=[1,11,31,48,65,73,81,90].filter(n=>n<=current).pop();document.querySelectorAll('[data-section]').forEach(b=>{const active=Number(b.dataset.section)===sectionStart;b.classList.toggle('active',active);b.setAttribute('aria-current',active?'true':'false');});
 byId('chapter').value=String(sectionStart);
